...
func _process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("Dialogue") and textline == 2:
		$TextboxContainer/MarginContainer/HBoxContainer/Label.set_text("Humanity took its place among the stars. But under the lazy rule of the Terran Parliament, became fractured and weak. And humanity suffered for it.")
		$TextboxContainer/TweenAnimation.play("TextTween")
