...
func _process(delta: float) -> void:
	if Input.is_action_just_pressed("Dialogue") and textline == 1:
		$TextboxContainer/MarginContainer/HBoxContainer/Label.set_text("Farseeing Couriers gave me this model as a reward for my service.")
		$TextboxContainer/TweenAnimation.play("TextTween")
		$CalihanSpeech.play()


	if Input.is_action_just_pressed("Dialogue") and textline == 2:
		$TextboxContainer/MarginContainer/HBoxContainer/Label.set_text("A bonus would've been nice, but the model is pretty cool too.")
		$TextboxContainer/TweenAnimation.play("TextTween")
		$CalihanSpeech.play()
